...
use crate::{tx::PaymentOutputs, utils::jio_to_sompi};
...
impl From<Jio> for Sompi {
    fn from(jio: Jio) -> Self {
        Sompi(jio_to_sompi(jio.0))
    }
}
    ...
    F: FnOnce(NetworkType) -> Address,
{
    ...
    let utxo_entries: Vec<UtxoEntryReference> = values.into_iter().map(jio_to_sompi).map(UtxoEntryReference::simulated).collect();
...
pub(crate) fn change_address(network_type: NetworkType) -> Address {
    match network_type {
        NetworkType::Mainnet => Address::try_from("jio:qpauqsvk7yf9unexwmxsnmg547mhyga37csh0kj53q6xxgl24ydxjsgzthw5j").unwrap(),
        NetworkType::Testnet => Address::try_from("jiotest:qqz22l98sf8jun72rwh5rqe2tm8lhwtdxdmynrz4ypwak427qed5juktjt7ju").unwrap(),
...
pub(crate) fn output_address(network_type: NetworkType) -> Address {
    match network_type {
        NetworkType::Mainnet => Address::try_from("jio:qrd9efkvg3pg34sgp6ztwyv3r569qlc43wa5w8nfs302532dzj47knu04aftm").unwrap(),
        NetworkType::Testnet => Address::try_from("jiotest:qqrewmx4gpuekvk8grenkvj2hp7xt0c35rxgq383f6gy223c4ud5s58ptm6er").unwrap(),
        _ => unreachable!("network type not supported"),
    }
}
...
fn test_generator_sweep_two_utxos() -> Result<()> {
    make_generator(test_network_id(), &[10.0, 10.0], &[], Fees::None, change_address, PaymentDestination::Change)
        ...
        .fetch(&Expected {
            is_final: true,
            input_count: 2,
            aggregate_input_value: Jio(20.0),
            output_count: 1,
...
fn test_generator_sweep_two_utxos_with_priority_fees_rejection() -> Result<()> {
    let generator =
        make_generator(test_network_id(), &[10.0, 10.0], &[], Fees::sender(Jio(5.0)), change_address, PaymentDestination::Change);
    match generator {
...
fn test_generator_compound_200k_10kas_transactions() -> Result<()> {
    generator(test_network_id(), &[10.0; 200_000], &[], Fees::sender(Jio(5.0)), [(output_address, Jio(190_000.0))].as_slice())
        .unwrap()
        .harness()
        .validate()
        .finalize();
...
fn test_generator_compound_100k_random_transactions() -> Result<()> {
    ...
    let outputs = [(output_address, Jio(total - 10.0))];
    generator(test_network_id(), &inputs, &[], Fees::sender(Jio(5.0)), outputs.as_slice()).unwrap().harness().validate().finalize();
...
fn test_generator_random_outputs() -> Result<()> {
    ...
    let outputs: Vec<_> = outputs.into_iter().map(|v| (output_address, Jio(v))).collect();

    generator(test_network_id(), &[total + 100.0], &[], Fees::sender(Jio(5.0)), outputs.as_slice())
        .unwrap()
        .harness()
        .validate()
        .finalize();
...
fn test_generator_dust_1_1() -> Result<()> {
    generator(
        ...
        &[],
        Fees::sender(Jio(5.0)),
        [(output_address, Jio(1.0)), (output_address, Jio(1.0))].as_slice(),
    )
    .unwrap()
    .harness()
    .fetch(&Expected {
        is_final: true,
        input_count: 4,
        aggregate_input_value: Jio(40.0),
        output_count: 3,
        priority_fees: FeesExpected::sender(Jio(5.0)),
    })
    .finalize();
...
fn test_generator_inputs_2_outputs_2_fees_exclude() -> Result<()> {
    generator(
        ...
        &[],
        Fees::sender(Jio(5.0)),
        [(output_address, Jio(10.0)), (output_address, Jio(1.0))].as_slice(),
    )
    .unwrap()
    .harness()
    .fetch(&Expected {
...
fn test_generator_inputs_100_outputs_1_fees_exclude_success() -> Result<()> {
    // generator(test_network_id(), &[10.0; 100], &[], Fees::sender(Jio(5.0)), [(output_address, Jio(990.0))].as_slice())
    generator(test_network_id(), &[10.0; 100], &[], Fees::sender(Jio(0.0)), [(output_address, Jio(990.0))].as_slice())
        .unwrap()
        .harness()
        .fetch(&Expected {
            is_final: false,
            input_count: 88,
            aggregate_input_value: Jio(880.0),
            output_count: 1,
            priority_fees: FeesExpected::None,
        })
        .fetch(&Expected {
            is_final: false,
            input_count: 12,
            aggregate_input_value: Jio(120.0),
            output_count: 1,
            priority_fees: FeesExpected::None,
        })
        .fetch(&Expected {
            ...
            output_count: 2,
            // priority_fees: FeesExpected::sender(Jio(5.0)),
            priority_fees: FeesExpected::sender(Jio(0.0)),
        })
        .finalize();
...
fn test_generator_inputs_100_outputs_1_fees_include_success() -> Result<()> {
    generator(
        ...
        &[1.0; 100],
        &[],
        Fees::receiver(Jio(5.0)),
        // [(output_address, Jio(100.0))].as_slice(),
        [(output_address, Jio(100.0))].as_slice(),
    )
    .unwrap()
    .harness()
    .fetch(&Expected {
        is_final: false,
        input_count: 88,
        aggregate_input_value: Jio(88.0),
        output_count: 1,
        priority_fees: FeesExpected::None,
    })
    .fetch(&Expected {
        is_final: false,
        input_count: 12,
        aggregate_input_value: Jio(12.0),
        output_count: 1,
        priority_fees: FeesExpected::None,
    })
    .fetch(&Expected {
        ...
        output_count: 1,
        priority_fees: FeesExpected::receiver(Jio(5.0)),
    })
    .finalize();
...
fn test_generator_inputs_100_outputs_1_fees_exclude_insufficient_funds() -> Result<()> {
    generator(test_network_id(), &[10.0; 100], &[], Fees::sender(Jio(5.0)), [(output_address, Jio(1000.0))].as_slice())
        .unwrap()
        .harness()
        .fetch(&Expected {
            is_final: false,
...
fn test_generator_inputs_1k_outputs_2_fees_exclude() -> Result<()> {
    generator(test_network_id(), &[10.0; 1_000], &[], Fees::sender(Jio(5.0)), [(output_address, Jio(9_000.0))].as_slice())
        .unwrap()
        .harness()
        .drain(
            10,
        ...
        .fetch(&Expected {
            is_final: false,
            input_count: 21,
            aggregate_input_value: Jio(210.0),
            output_count: 1,
            priority_fees: FeesExpected::None,
        })
        .fetch(&Expected {
...
fn test_generator_inputs_32k_outputs_2_fees_exclude() -> Result<()> {
    ...
    generator(
        ...
        &[f; 32_747],
        &[],
        Fees::sender(Jio(10_000.0)),
        [(output_address, Jio(f * 32_747.0 - 10_001.0))].as_slice(),
    )
    .unwrap()
    .harness()
    .accumulate(379)
