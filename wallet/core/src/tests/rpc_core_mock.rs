...
use jio_notify::events::EVENT_TYPE_ARRAY;
use jio_notify::listener::{ListenerId, ListenerLifespan};
use jio_notify::notifier::{Notifier, Notify};
use jio_notify::scope::Scope;
...
use jio_rpc_core::api::ctl::RpcCtl;
use jio_rpc_core::{api::connection::DynRpcConnection, api::rpc::RpcApi, *};
use jio_rpc_core::{notify::connection::ChannelConnection, RpcResult};
use std::sync::Arc;
...
impl RpcCoreMock {
    ...
    #[allow(dead_code)]
    pub fn notify_new_block_template(&self) -> jio_notify::error::Result<()> {
