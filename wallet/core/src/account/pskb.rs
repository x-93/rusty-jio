...
use futures::stream;
use jio_bip32::{DerivationPath, KeyFingerprint, PrivateKey};
use jio_consensus_client::UtxoEntry as ClientUTXO;
use jio_consensus_core::hashing::sighash::{calc_schnorr_signature_hash, SigHashReusedValues};
use jio_consensus_core::tx::VerifiableTransaction;
use jio_consensus_core::tx::{TransactionInput, UtxoEntry};
use jio_txscript::extract_script_pub_key_address;
use jio_txscript::opcodes::codes::OpData65;
use jio_txscript::script_builder::ScriptBuilder;
use jio_wallet_core::tx::{Generator, GeneratorSettings, PaymentDestination, PendingTransaction};
pub use jio_wallet_pskt::bundle::Bundle;
use jio_wallet_pskt::prelude::KeySource;
use jio_wallet_pskt::prelude::{Finalizer, Inner, SignInputOk, Signature, Signer};
pub use jio_wallet_pskt::pskt::{Creator, PSKT};
...
pub fn pskt_to_pending_transaction(
    ...
    let output: Vec<jio_consensus_core::tx::TransactionOutput> = signed_tx.outputs.clone();
    ...
    let pending_tx = PendingTransaction::try_new(
        ...
        0,
        0,
        jio_wallet_core::tx::DataKind::Final,
    )?;
