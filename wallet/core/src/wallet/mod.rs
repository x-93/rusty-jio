//!
//! Jio wallet runtime implementation.
//!
pub mod api;
pub mod args;
pub mod maps;
...
use jio_bip32::{ExtendedKey, Language, Mnemonic, Prefix as KeyPrefix, WordCount};
use jio_notify::{
    listener::ListenerId,
    scope::{Scope, VirtualDaaScoreChangedScope},
};
use jio_wallet_keys::xpub::NetworkTaggedXpub;
use jio_wrpc_client::{JioRpcClient, Resolver, WrpcEncoding};
...
impl Wallet {
    ...
    pub fn try_wrpc_client(&self) -> Option<Arc<JioRpcClient>> {
        self.try_rpc_api().and_then(|api| api.clone().downcast_arc::<JioRpcClient>().ok())
    }

    pub fn wrpc_client(&self) -> Arc<JioRpcClient> {
        self.try_rpc_api().and_then(|api| api.clone().downcast_arc::<JioRpcClient>().ok()).unwrap()
    }
    ...
    pub fn address_prefix(&self) -> Result<jio_addresses::Prefix> {
    ...
    pub async fn import_jiowallet_golang_single_v1<T: AsRef<[u8]>>(
        self: &Arc<Wallet>,
        ...
        let prefix = file.xpublic_key.split_at(jio_bip32::Prefix::LENGTH).0;
        let prefix = jio_bip32::Prefix::try_from(prefix)?;
    ...
    pub async fn import_jiowallet_golang_single_v0<T: AsRef<[u8]>>(
        self: &Arc<Wallet>,
        import_secret: &Secret,
        wallet_secret: &Secret,
    ...
    pub async fn import_jiowallet_golang_multisig_v0<T: AsRef<[u8]>>(
        self: &Arc<Wallet>,
        import_secret: &Secret,
        wallet_secret: &Secret,
        ...
        let prefix = first_pub_key.split_at(jio_bip32::Prefix::LENGTH).0;
        let prefix = jio_bip32::Prefix::try_from(prefix)?;
    ...
    pub async fn import_jiowallet_golang_multisig_v1<T: AsRef<[u8]>>(
        self: &Arc<Wallet>,
        import_secret: &Secret,
        wallet_secret: &Secret,
        ...
        all_pub_keys.sort_unstable_by(|left, right| {
            left.split_at(jio_bip32::Prefix::LENGTH).1.cmp(right.split_at(jio_bip32::Prefix::LENGTH).1)
        });
    ...
    pub async fn import_multisig_with_mnemonic(
        ...
        let mut additional_xpub_keys = additional_xpub_keys
            ...
            .collect::<Result<Vec<_>, jio_bip32::Error>>()?;
...
mod test {
    ...
    use jio_addresses::{Prefix, Version};
    use jio_bip32::{ChildNumber, ExtendedPrivateKey, SecretKey};
    use jio_consensus_core::subnets::SUBNETWORK_ID_NATIVE;
    use jio_consensus_wasm::{sign_transaction, SignableTransaction, Transaction, TransactionInput, TransactionOutput};
    ...
    async fn wallet_test() -> Result<()> {
        ...
        let address = Address::try_from("jiotest:qz7ulu4c25dh7fzec9zjyrmlhnkzrg4wmf89q7gzr3gfrsj3uz6xjceef60sd")?;
        ...
        let to_address = Address::try_from("jiotest:qpakxqlesqywgkq7rg4wyhjd93kmw7trkl3gpa3vd5flyt59a43yyn8vu0w8c")?;
        ...
        let derivation_path =
            gen1::WalletDerivationManager::build_derivate_path(false, 0, None, Some(jio_bip32::AddressType::Receive))?;
