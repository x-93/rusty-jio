...
use jio_wrpc_client::JioRpcClient;
...
impl UtxoProcessor {
    ...
    pub fn rpc_client(&self) -> Option<Arc<JioRpcClient>> {
        self.rpc_api().clone().downcast_arc::<JioRpcClient>().ok()
    }
    ...
    pub async fn init_state_from_server(&self) -> Result<bool> {
        ...
        log_trace!("Connected to jiopad: '{server_version}' on '{server_network_id}';  SYNC: {is_synced}  DAA: {virtual_daa_score}");
