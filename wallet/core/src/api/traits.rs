//!
//! API trait for interfacing with the Jio wallet subsystem.
//!
...
pub trait WalletApi: Send + Sync + AnySync {
    ...
    /// If the `filename` is `None`, the wallet opens the default wallet named `jio`.
    ///
    ...
    /// applications such as KDX and jionet.io web wallet using a deprecated derivation path
