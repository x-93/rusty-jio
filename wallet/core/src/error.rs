...
use jio_consensus_core::sign::Error as CoreSignError;
...
pub enum Error {
    ...
    #[error(transparent)]
    WalletKeys(#[from] jio_wallet_keys::error::Error),
    ...
    #[error("{0}")]
    NetworkType(#[from] jio_consensus_core::network::NetworkTypeError),

    #[error("{0}")]
    NetworkId(#[from] jio_consensus_core::network::NetworkIdError),
    ...
    #[error(transparent)]
    Address(#[from] jio_addresses::AddressError),
    ...
    #[error(transparent)]
    ScriptBuilderError(#[from] jio_txscript::script_builder::ScriptBuilderError),
    ...
    #[error(transparent)]
    ConsensusClient(#[from] jio_consensus_client::error::Error),

    #[error(transparent)]
    ConsensusWasm(#[from] jio_consensus_wasm::error::Error),
    ...
    #[error(transparent)]
    MultisigCreateError(#[from] jio_txscript::MultisigCreateError),

    #[error(transparent)]
    TxScriptError(#[from] jio_txscript_errors::TxScriptError),
    ...
    #[error(transparent)]
    Metrics(#[from] jio_metrics_core::error::Error),

    #[error("Connected node is not synced")]
    NotSynced,
    #[error(transparent)]
    Pskt(#[from] jio_wallet_pskt::error::Error),
