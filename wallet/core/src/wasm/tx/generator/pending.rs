...
use jio_consensus_client::{numeric, string};
use jio_consensus_client::{Transaction, TransactionT};
use jio_consensus_core::hashing::wasm::SighashType;
use jio_wallet_keys::privatekey::PrivateKey;
use jio_wasm_core::types::{BinaryT, HexString};
use jio_wrpc_wasm::RpcClient;
    ...
    pub fn sign(&self, js_value: PrivateKeyArrayT, check_fully_signed: Option<bool>) -> Result<()> {
        if let Ok(keys) = js_value.dyn_into::<Array>() {
            let keys = keys
                ...
                .collect::<std::result::Result<Vec<_>, jio_wallet_keys::error::Error>>()?;
