...
// use jio_bip32::Prefix;
use jio_consensus_core::network::{NetworkId, NetworkType};
use jio_consensus_core::tx::{ScriptPublicKey, TransactionOutpoint, UtxoEntry};

use hex;
use jio_txscript::{extract_script_pub_key_address, pay_to_address_script, pay_to_script_hash_script};
...
pub fn script_sig_to_address(script_sig: &[u8], prefix: jio_addresses::Prefix) -> Result<Address, Error> {
...
mod tests {
    ...
    use crate::role::*;
    use jio_consensus_core::tx::{TransactionId, TransactionOutpoint, UtxoEntry};
    use jio_txscript::{multisig_redeem_script, pay_to_script_hash_script};
