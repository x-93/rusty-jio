use crate::{common::ProtocolError, pb::JiopadMessage, ConnectionInitializer, Peer, Router};
use jio_core::{debug, info, warn};
...
impl Hub {
    ...
    pub async fn send(&self, peer_key: PeerKey, msg: JiopadMessage) -> Result<bool, ProtocolError> {
    ...
    pub async fn broadcast(&self, msg: JiopadMessage) {
    ...
    pub async fn broadcast_to_some_peers(&self, msg: JiopadMessage, num_peers: usize) {
        assert!(num_peers > 0);
    ...
    pub async fn broadcast_many(&self, msgs: Vec<JiopadMessage>) {
        if msgs.is_empty() {
            return;
        }
        let peers = self.peers.read().values().cloned().collect::<Vec<_>>();
