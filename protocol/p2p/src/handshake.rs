...
use crate::pb::{jiopad_message::Payload, ReadyMessage, VerackMessage, VersionMessage};
use crate::{common::ProtocolError, dequeue_with_timeout, make_message};
use crate::{IncomingRoute, JiopadMessagePayloadType, Router};
use jio_core::debug;

/// Implements the Jio peer-to-peer handshake protocol
pub struct JiopadHandshake<'a> {
    router: &'a Router,
...
impl<'a> JiopadHandshake<'a> {
    /// Builds the handshake object and subscribes to handshake messages
    pub fn new(router: &'a Router) -> Self {
        Self {
            router,
            version_receiver: router.subscribe(vec![JiopadMessagePayloadType::Version]),
            verack_receiver: router.subscribe(vec![JiopadMessagePayloadType::Verack]),
            ready_receiver: router.subscribe(vec![JiopadMessagePayloadType::Ready]),
        }
    }
