use crate::{
    ...
    handshake::JiopadHandshake,
    pb::{self, VersionMessage},
    IncomingRoute, JiopadMessagePayloadType, Router,
};
use jio_core::{debug, time::unix_now, trace, warn};
use std::sync::Arc;
...
impl EchoFlow {
    pub async fn register(router: Arc<Router>) {
        ...
        let receiver = router.subscribe(vec![
            ...
            JiopadMessagePayloadType::RequestAntipast,
    ...
    async fn call(&self, msg: pb::JiopadMessage) -> bool {
        // echo
...
impl ConnectionInitializer for EchoFlowInitializer {
    async fn initialize_connection(&self, router: Arc<Router>) -> Result<(), ProtocolError> {
        //
        // Example code to illustrate jio P2P handshaking
        //
...
mod tests {
    ...
    use jio_core::debug;
    use jio_utils::networking::NetAddress;

    #[tokio::test]
    async fn test_handshake() {
        jio_core::log::try_init_logger("debug");
