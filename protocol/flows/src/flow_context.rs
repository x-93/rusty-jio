...
use jio_consensus_core::api::{BlockValidationFuture, BlockValidationFutures};
use jio_consensus_core::block::Block;
use jio_consensus_core::config::Config;
...
use jio_consensusmanager::{BlockProcessingBatch, ConsensusInstance, ConsensusManager, ConsensusProxy};
use jio_core::{
    debug, info,
    jiopad_env::{name, version},
    task::tick::TickService,
};
use jio_core::{time::unix_now, warn};
use jio_hashes::Hash;
use jio_mining::mempool::tx::{Orphan, Priority};
use jio_mining::{manager::MiningManagerProxy, mempool::tx::RbfPolicy};
use jio_notify::notifier::Notify;
use jio_p2p_lib::{
    common::ProtocolError,
    convert::model::version::Version,
    make_message,
    pb::{jiopad_message::Payload, InvRelayBlockMessage},
    ConnectionInitializer, Hub, JiopadHandshake, PeerKey, PeerProperties, Router,
};
use jio_utils::iter::IterExtensions;
...
impl FlowContext {
    ...
    ///
    /// _GO-JIOPAD: OnNewBlock + broadcastTransactionsAfterBlockAdded_
...
impl ConnectionInitializer for FlowContext {
    async fn initialize_connection(&self, router: Arc<Router>) -> Result<(), ProtocolError> {
        ...
        let mut handshake = JiopadHandshake::new(&router);
