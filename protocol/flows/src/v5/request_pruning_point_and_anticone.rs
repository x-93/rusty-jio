...
pub struct RequestPruningPointAndAnticoneFlow {
    ctx: Arc<FlowContext>,
    peer: Arc<Peer>,
    incoming_rx: Receiver<Vec<u8>>,
}

impl RequestPruningPointAndAnticoneFlow {
...
#[async_trait]
impl Flow for RequestPruningPointAndAnticoneFlow {
    async fn start(&mut self) -> Result<(), String> {
        while let Some(_msg) = self.incoming_rx.recv().await {
            // Serve pruning point and anticone
        }
        Ok(())
    }
}
