...
pub struct RequestAntipastFlow {
    ctx: Arc<FlowContext>,
    peer: Arc<Peer>,
    incoming_rx: Receiver<Vec<u8>>,
}

impl RequestAntipastFlow {
    pub fn new(ctx: Arc<FlowContext>, peer: Arc<Peer>) -> Self {
        let incoming_rx = peer.router.subscribe(vec![JioPayloadType::RequestAntipast]);
        Self {
            ctx,
            peer,
            incoming_rx,
...
#[async_trait]
impl Flow for RequestAntipastFlow {
