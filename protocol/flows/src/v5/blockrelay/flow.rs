...
use jio_consensus_core::{api::BlockValidationFutures, block::Block, blockstatus::BlockStatus, errors::block::RuleError};
use jio_consensusmanager::{BlockProcessingBatch, ConsensusProxy};
use jio_core::debug;
use jio_hashes::Hash;
use jio_p2p_lib::{
    common::ProtocolError,
    dequeue, dequeue_with_timeout, make_message, make_request,
    pb::{jiopad_message::Payload, InvRelayBlockMessage, RequestBlockLocatorMessage, RequestRelayBlocksMessage},
    IncomingRoute, Router, SharedIncomingRoute,
};
use jio_utils::channel::{JobSender, JobTrySendError as TrySendError};
...
impl HandleRelayInvsFlow {
    ...
    async fn check_orphan_resolution_range(
        ...
        // with current syncer-side implementations (in both go-jio and this codebase) we could query only the last one,
