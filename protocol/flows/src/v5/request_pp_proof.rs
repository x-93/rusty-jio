...
pub struct RequestPpProofFlow {
    ctx: Arc<FlowContext>,
    peer: Arc<Peer>,
    incoming_rx: Receiver<Vec<u8>>,
}

impl RequestPpProofFlow {
    pub fn new(ctx: Arc<FlowContext>, peer: Arc<Peer>) -> Self {
        let incoming_rx = peer.router.subscribe(vec![JioPayloadType::RequestPruningPointProof]);
        Self {
            ctx,
            peer,
            incoming_rx,
...
#[async_trait]
impl Flow for RequestPpProofFlow {
    async fn start(&mut self) -> Result<(), String> {
        while let Some(_msg) = self.incoming_rx.recv().await {
            // Serve pruning point proof
        }
        Ok(())
    }
}
