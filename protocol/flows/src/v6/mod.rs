...
use jio_p2p_lib::{JiopadMessagePayloadType, Router, SharedIncomingRoute};
use jio_utils::channel;
use std::sync::Arc;
...
pub fn register(ctx: FlowContext, router: Arc<Router>) -> Vec<Box<dyn Flow>> {
    ...
    let mut flows: Vec<Box<dyn Flow>> = vec![
        ...
        Box::new(SendPingsFlow::new(ctx.clone(), router.clone(), router.subscribe(vec![JiopadMessagePayloadType::Pong]))),
        ...
        Box::new(HandleAntipastRequests::new(
            ctx.clone(),
            router.clone(),
            router.subscribe(vec![JiopadMessagePayloadType::RequestAntipast]),
        )),
        Box::new(RelayTransactionsFlow::new(
            ...
            router.subscribe_with_capacity(
                vec![JiopadMessagePayloadType::Transaction, JiopadMessagePayloadType::TransactionNotFound],
    ...
    let invs_route = router.subscribe_with_capacity(vec![JiopadMessagePayloadType::InvRelayBlock], ctx.block_invs_channel_size());
