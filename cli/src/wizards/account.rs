use crate::cli::JioCli;
use crate::imports::*;
use crate::result::Result;
use jio_bip32::{Language, Mnemonic, WordCount};
use jio_wallet_core::account::MULTISIG_ACCOUNT_KIND;
// use jio_wallet_core::runtime::wallet::AccountCreateArgsBip32;
// use jio_wallet_core::runtime::{PrvKeyDataArgs, PrvKeyDataCreateArgs};
// use jio_wallet_core::storage::AccountKind;
...
async fn create_multisig(ctx: &Arc<JioCli>, account_name: Option<String>, mnemonic_phrase_word_count: WordCount) -> Result<()> {
    let term = ctx.term();
    let wallet = ctx.wallet();
...
pub(crate) async fn bip32_watch(ctx: &Arc<JioCli>, name: Option<&str>) -> Result<()> {
    let term = ctx.term();
    let wallet = ctx.wallet();
...
pub(crate) async fn multisig_watch(ctx: &Arc<JioCli>, name: Option<&str>) -> Result<()> {
    let term = ctx.term();
