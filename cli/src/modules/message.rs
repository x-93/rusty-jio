use jio_addresses::Version;
use jio_bip32::secp256k1::XOnlyPublicKey;
use jio_wallet_core::{
...
impl Message {
    async fn main(self: Arc<Self>, ctx: Arc<JioCli>, argv: Vec<String>, _cmd: &str) -> Result<()> {
        if argv.is_empty() {
        ...
        match argv.first().unwrap().as_str() {
            ...
            "verify" => {
                ...
                self.verify(ctx, jio_address, signature, message).await?;
            }
            v => {
    ...
    async fn display_help(self: Arc<Self>, ctx: Arc<JioCli>, _argv: Vec<String>) -> Result<()> {
        ctx.term().help(
            &[
                ("sign <jio_address>", "Sign a message with the private key that matches the given address. Prompts for message."),
                (
                    "verify <jio_address> <signature>",
                    "Verify the signature against the message and jio_address. Prompts for message.",
                ),
            ],
            None,
        )?;
    ...
    async fn sign(self: Arc<Self>, ctx: Arc<JioCli>, jio_address: &str, message: &str) -> Result<()> {
        let jio_address = Address::try_from(jio_address)?;
        if jio_address.version != Version::PubKey {
        ...
        let privkey = self.get_address_private_key(&ctx, jio_address).await?;
    ...
    async fn verify(self: Arc<Self>, ctx: Arc<JioCli>, jio_address: &str, signature: &str, message: &str) -> Result<()> {
        let jio_address = Address::try_from(jio_address)?;
        if jio_address.version != Version::PubKey {
            return Err(Error::custom("Address not supported for message signing. Only supports PubKey addresses"));
        }

        let pubkey = XOnlyPublicKey::from_slice(&jio_address.payload[0..32]).unwrap();
    ...
    async fn get_address_private_key(self: Arc<Self>, ctx: &Arc<JioCli>, jio_address: Address) -> Result<[u8; 32]> {
        ...
        match account.account_kind().as_ref() {
            BIP32_ACCOUNT_KIND => {
                ...
                let (receive, change) = account.derivation().addresses_indexes(&[&jio_address])?;
