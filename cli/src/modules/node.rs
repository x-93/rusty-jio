...
pub struct Node {
    settings: SettingsStore<JiopadSettings>,
    mute: Arc<AtomicBool>,
...
impl Default for Node {
    fn default() -> Self {
        Node {
            settings: SettingsStore::try_new("jiopad").expect("Failed to create node settings store"),
...
impl Handler for Node {
    fn verb(&self, ctx: &Arc<dyn Context>) -> Option<&'static str> {
        if let Ok(ctx) = ctx.clone().downcast_arc::<JioCli>() {
            ctx.daemons().clone().jiopad.as_ref().map(|_| "node")
        } else {
            None
        }
    }
    ...
    async fn handle(self: Arc<Self>, ctx: &Arc<dyn Context>, argv: Vec<String>, cmd: &str) -> cli::Result<()> {
        let ctx = ctx.clone().downcast_arc::<JioCli>()?;
...
impl Node {
    ...
    async fn create_config(&self, ctx: &Arc<JioCli>) -> Result<JiopadConfig> {
        let location: String = self
            .settings
            .get(JiopadSettings::Location)
        ...
        let mute = false;
        let config = JiopadConfig::new(location.as_str(), network_id, mute);
        Ok(config)
    }

    async fn main(self: Arc<Self>, ctx: Arc<JioCli>, mut argv: Vec<String>, cmd: &str) -> Result<()> {
        if argv.is_empty() {
            return self.display_help(ctx, argv).await;
        }
        let jiopad = ctx.daemons().jiopad();
        match argv.remove(0).as_str() {
            "start" => {
                let mute = self.mute.load(Ordering::SeqCst);
                if mute {
                    tprintln!(ctx, "starting jio node... {}", style("(logs are muted, use 'node mute' to toggle)").dim());
                } else {
                    tprintln!(ctx, "starting jio node... {}", style("(use 'node mute' to mute logging)").dim());
                }

                let wrpc_client = ctx.wallet().try_wrpc_client().ok_or(Error::custom("Unable to start node with non-wRPC client"))?;

                jiopad.configure(self.create_config(&ctx).await?).await?;
                jiopad.start().await?;
            ...
            "mute" | "logs" => {
                ...
                // jiopad.mute(mute).await?;
                self.settings.set(JiopadSettings::Mute, mute).await?;
            }
            "status" => {
                let status = jiopad.status().await?;
                tprintln!(ctx, "{}", status);
            }
            "select" => {
            ...
            "version" => {
                jiopad.configure(self.create_config(&ctx).await?).await?;
                let version = jiopad.version().await?;
    ...
    async fn display_help(self: Arc<Self>, ctx: Arc<JioCli>, _argv: Vec<String>) -> Result<()> {
        ctx.term().help(
            &[
                ("select", "Select Jiopad executable (binary) location"),
                ("version", "Display Jiopad executable version"),
                ("start", "Start the local Jio node instance"),
                ("stop", "Stop the local Jio node instance"),
                ("restart", "Restart the local Jio node instance"),
                ("kill", "Kill the local Jio node instance"),
                ("status", "Get the status of the local Jio node instance"),
    ...
    async fn select(self: Arc<Self>, ctx: Arc<JioCli>, path: Option<String>) -> Result<()> {
        let root = nw_sys::app::folder();

        match path {
            None => {
                let binaries = jio_daemon::locate_binaries(root.as_str(), "jiopad").await?;

                if binaries.is_empty() {
                    tprintln!(ctx, "No jiopad binaries found");
                } else {
                    let binaries = binaries.iter().map(|p| p.display().to_string()).collect::<Vec<_>>();
                    if let Some(selection) = ctx.term().select("Please select a jiopad binary", &binaries).await? {
                        tprintln!(ctx, "selecting: {}", selection);
                        self.settings.set(JiopadSettings::Location, selection.as_str()).await?;
                    } else {
            ...
            Some(path) => {
                if fs::exists(&path).await? {
                    ...
                    self.settings.set(JiopadSettings::Location, path.as_str()).await?;
                } else {
                    twarnln!(ctx, "destination binary not found, please specify full path including the binary name");
                    twarnln!(ctx, "example: 'node select /home/user/testnet/jiopad'");
                    tprintln!(ctx, "no selection is made");
                }
            }
        }
    ...
    pub async fn handle_event(&self, ctx: &Arc<JioCli>, event: Event) -> Result<()> {
        let term = ctx.term();
        ...
        match event {
            ...
            Event::Exit(_code) => {
                tprintln!(ctx, "Jiopad has exited");
                self.is_running.store(false, Ordering::SeqCst);
                term.refresh_prompt();
            }
            Event::Error(error) => {
                tprintln!(ctx, "{}", style(format!("Jiopad error: {error}")).red());
                self.is_running.store(false, Ordering::SeqCst);
                term.refresh_prompt();
            }
