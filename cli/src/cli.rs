...
use jio_daemon::{DaemonEvent, DaemonKind, Daemons};
use jio_wallet_core::account::Account;
use jio_wallet_core::rpc::DynRpcApi;
use jio_wallet_core::storage::{IdT, PrvKeyDataInfo};
use jio_wrpc_client::{JioRpcClient, Resolver};
...
impl From<&JioCli> for Arc<Terminal> {
    fn from(ctx: &JioCli) -> Arc<Terminal> {
        ctx.term()
    }
}

impl AsRef<JioCli> for JioCli {
    fn as_ref(&self) -> &Self {
        self
    }
}

impl workflow_log::Sink for JioCli {
...
impl JioCli {
    pub fn init() {
        cfg_if! {
            if #[cfg(not(target_arch = "wasm32"))] {
                ...
                jio_core::log::init_logger(None, "info");
            } else {
                jio_core::log::set_log_level(LevelFilter::Info);
            }
        }
    ...
    pub async fn try_new_arc(options: Options) -> Result<Arc<Self>> {
        ...
        let term = Arc::new(Terminal::try_new_with_options(jio_cli.clone(), options.terminal)?);
        term.init().await?;

        cfg_if! {
            if #[cfg(target_arch = "wasm32")] {
                jio_cli.init_panic_hook();
            }
        }
    ...
    pub fn try_rpc_client(&self) -> Option<Arc<JioRpcClient>> {
                ...
                select! {
                    ...
                    msg = multiplexer.receiver.recv().fuse() => {
                        ...
                        if let Ok(msg) = msg {
                            match *msg {
                                Events::WalletPing => {
                                    // log_info!("Jio NG - received wallet ping");
                                },
                                Events::Metrics { network_id : _, metrics : _ } => {
                                    // log_info!("Jio NG - received metrics event {metrics:?}")
                                }
                                ...
                                Events::UtxoIndexNotEnabled { .. } => {
                                    tprintln!(this, "Error: Jio node UTXO index is not enabled...")
                                },
                                ...
                                Events::ServerStatus {
                                    ...
                                    tprintln!(this, "Connected to Jio node version {server_version} at {}", url.unwrap_or("N/A".to_string()));

                                    let is_open = this.wallet.is_open();

                                    if !is_synced {
                                        if is_open {
                                            terrorln!(this, "Unable to update the wallet state - Jio node is currently syncing with the network...");

                                        } else {
                                            terrorln!(this, "Jio node is currently syncing with the network, please wait for the sync to complete...");
                                        }
                                    }
    ...
    pub async fn shutdown(&self) -> Result<()> {
        if !self.shutdown.load(Ordering::SeqCst) {
            ...
            let jiopad = self.daemons().try_jiopad();
            ...
            if let Some(jiopad) = jiopad.as_ref() {
                jiopad.mute(false).await?;
                jiopad.stop().await?;
            }
...
impl Cli for JioCli {
    ...
    fn prompt(&self) -> Option<String> {
        ...
        if let Some(descriptor) = self.wallet.descriptor() {
            ...
            if title.to_lowercase().as_str() != "jio" {
                prompt.push(title);
            }
...
pub async fn jio_cli(terminal_options: TerminalOptions, banner: Option<String>) -> Result<()> {
    JioCli::init();

    let options = Options::new(terminal_options, None);
    let cli = JioCli::try_new_arc(options).await?;

    let banner =
        banner.unwrap_or_else(|| format!("Jio Cli Wallet v{} (type 'help' for list of commands)", env!("CARGO_PKG_VERSION")));
