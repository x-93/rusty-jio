...
pub use jiopad::{Jiopad, JiopadConfig, JiopadCtl};
...
pub static LOCATIONS: &[&str] = &[
    ...
    "../target/release",
    "../target/debug",
    "../../jio-cpu-miner/target/debug",
    "../../jio-cpu-miner/target/release",
    "bin/windows-x64",
    "bin/linux-ia32",
    "bin/linux-x64",
    "bin/linux-arm64",
...
#[derive(Default)]
pub struct Daemons {
    pub jiopad: Option<Arc<dyn JiopadCtl + Send + Sync + 'static>>,
    // pub jiopad_automute : Arc<
    pub cpu_miner: Option<Arc<dyn CpuMinerCtl + Send + Sync + 'static>>,
}

impl Daemons {
    pub fn new() -> Self {
        Self { jiopad: None, cpu_miner: None }
    }

    pub fn with_jiopad(mut self, jiopad: Arc<dyn JiopadCtl + Send + Sync + 'static>) -> Self {
        self.jiopad = Some(jiopad);
        self
    }
    ...
    pub fn jiopad(&self) -> Arc<dyn JiopadCtl + Send + Sync + 'static> {
        self.jiopad.as_ref().expect("accessing Daemons::jiopad while jiopad option is None").clone()
    }

    pub fn try_jiopad(&self) -> Option<Arc<dyn JiopadCtl + Send + Sync + 'static>> {
        self.jiopad.clone()
    }
