...
impl TryFrom<JiopadConfig> for Vec<String> {
    type Error = Error;
    fn try_from(args: JiopadConfig) -> Result<Vec<String>> {
        let mut argv = Vec::new();

        if args.path.is_none() {
            return Err(Error::Custom("no jiopad path is specified".to_string()));
        }
...
impl Jiopad {
    pub fn new(args: JiopadConfig) -> Self {
        Self {
    ...
    pub fn configure(&self, config: JiopadConfig) -> Result<()> {
    ...
    pub fn start(&self) -> Result<()> {
        ...
        if let Some(process) = process {
            if process.is_running() {
                return Err(Error::Custom("Jio node is already running.".to_string()));
            }
        }
        ...
        // let options = JiopadOptions::new(path,network)?;
    ...
    pub async fn version(&self) -> Result<String> {
        ...
        if let Some(path) = path {
            ...
            Ok("Jiopad binary is not configured. Please use 'node select' command.".to_string())
        }
    }
}
...
pub trait JiopadCtl {
    ...
    async fn configure(&self, config: JiopadConfig) -> Result<()>;
