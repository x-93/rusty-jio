...
use jio_notify::{events::EventType, scope::Scope, subscription::Command};
use jio_rpc_core::RpcResult;
use std::{
...
impl ListeningClient {
    pub async fn connect(jiopad: &Daemon) -> Self {
        let client = jiopad.new_multi_listener_client().await;
