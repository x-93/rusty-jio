...
use jio_consensus::params::SIMNET_PARAMS;
use jio_consensus_core::header::Header;
use jio_consensusmanager::ConsensusManager;
use jio_core::{task::runtime::AsyncRuntime, trace};
use jio_grpc_client::GrpcClient;
use jio_notify::scope::{BlockAddedScope, UtxosChangedScope, VirtualDaaScoreChangedScope};
use jio_rpc_core::{api::rpc::RpcApi, Notification, RpcTransactionId};
...
async fn daemon_sanity_test() {
    ...
    // let total_fd_limit =  jio_utils::fd_budget::get_limit() / 2 - 128;
    let total_fd_limit = 10;
    let mut jiopad1 = Daemon::new_random(total_fd_limit);
    let rpc_client1 = jiopad1.start().await;
    assert!(rpc_client1.handle_message_id() && rpc_client1.handle_stop_notify(), "the client failed to collect server features");

    let mut jiopad2 = Daemon::new_random(total_fd_limit);
    let rpc_client2 = jiopad2.start().await;
...
async fn daemon_mining_test() {
    ...
    let mut jiopad1 = Daemon::new_random_with_args(args.clone(), total_fd_limit);
    let mut jiopad2 = Daemon::new_random_with_args(args, total_fd_limit);
    let rpc_client1 = jiopad1.start().await;
    let rpc_client2 = jiopad2.start().await;

    rpc_client2.add_peer(format!("127.0.0.1:{}", jiopad1.p2p_port).try_into().unwrap(), true).await.unwrap();
    ...
    for i in 0..10 {
        let template = rpc_client1
            .get_block_template(Address::new(jiopad1.network.into(), jio_addresses::Version::PubKey, &[0; 32]), vec![])
            .await
            .unwrap();
    ...
    let vc = rpc_client2.get_virtual_chain_from_block(jio_consensus::params::SIMNET_GENESIS.hash, true).await.unwrap();
...
/// `cargo test --release --package jio-testing-integration --lib -- daemon_integration_tests::daemon_utxos_propagation_test`
#[tokio::test(flavor = "multi_thread", worker_threads = 1)]
async fn daemon_utxos_propagation_test() {
    #[cfg(feature = "heap")]
    let _profiler = dhat::Profiler::builder().file_name("jio-testing-integration-heap.json").build();

    jio_core::log::try_init_logger(
        "INFO,jio_testing_integration=trace,jio_notify=debug,jio_rpc_core=debug,jio_grpc_client=debug",
    );
    ...
    let miner_address =
        Address::new(jiopad1.network.into(), jio_addresses::Version::PubKey, &miner_pk.x_only_public_key().0.serialize());
    ...
    let user_address =
        Address::new(jiopad1.network.into(), jio_addresses::Version::PubKey, &user_pk.x_only_public_key().0.serialize());

    // Some dummy non-monitored address
    let blank_address = Address::new(jiopad1.network.into(), jio_addresses::Version::PubKey, &[0; 32]);
    ...
    let mut clients = vec![ListeningClient::connect(&jiopad2).await, ListeningClient::connect(&jiopad1).await];
...
async fn daemon_cleaning_test() {
    init_allocator_with_default_settings();
    jio_core::log::try_init_logger("info,jio_grpc_core=trace,jio_grpc_server=trace,jio_grpc_client=trace,jio_core=trace");
    ...
    {
        let total_fd_limit = 10;
        let mut jiopad1 = Daemon::new_random_with_args(args, total_fd_limit);
        let dyn_consensus_manager = jiopad1.core.find(ConsensusManager::IDENT).unwrap();
        let dyn_async_runtime = jiopad1.core.find(AsyncRuntime::IDENT).unwrap();
        ...
        core = Arc::downgrade(&jiopad1.core);
