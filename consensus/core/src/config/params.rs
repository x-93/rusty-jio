...
impl Params {
    ...
    /// Based on the analysis at <https://github.com/jionet/docs/blob/main/Reference/prunality/Prunality.pdf>
    ...
    pub fn genesis_hash(&self) -> jio_hashes::Hash {
        crate::hashing::header::header_hash(&self.genesis.header)
    }
...
pub const MAINNET_PARAMS: Params = Params {
    ...
    // This is technically a soft fork from the Go implementation since jiopad's consensus doesn't
...
mod tests {
    ...
    #[test]
    fn test_mainnet_params_properties() {
        let params = MAINNET_PARAMS;
        assert!(params.net.is_mainnet());
        assert_eq!(params.ghostdag_k, 18);
        assert_eq!(params.target_time_per_block, 1000);
        assert_eq!(params.bps(), 1);
        assert_eq!(params.network_name(), "mainnet");
        assert_eq!(params.default_p2p_port(), 16111);
        assert_eq!(params.default_rpc_port(), 16110);
    }

    #[test]
    fn test_testnet11_params_bps_scaling() {
        let params = TESTNET11_PARAMS;
        assert_eq!(params.target_time_per_block, 100);
        assert_eq!(params.bps(), 10);
        assert_eq!(params.sampling_activation_daa_score, 0);
        assert_eq!(params.network_name(), "testnet-11");
    }

    #[test]
    fn test_simnet_params() {
        let params = SIMNET_PARAMS;
        assert!(params.skip_proof_of_work);
        assert!(params.max_block_parents >= 64);
    }
}
