...
pub enum NetworkIdError {
    #[error("Invalid network name prefix: {0}. The expected prefix is 'jio'.")]
...
///
/// NetworkId is a unique identifier for a jio network instance.
...
impl NetworkId {
    ...
    /// Returns a textual description of the network prefixed with `jio-`
    pub fn to_prefixed(&self) -> String {
        format!("jio-{}", self)
    }

    pub fn from_prefixed(prefixed: &str) -> Result<Self, NetworkIdError> {
        if let Some(stripped) = prefixed.strip_prefix("jio-") {
