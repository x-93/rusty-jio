...
/// @category Consensus
#[derive(Clone, Debug, PartialEq, Eq, Default, Serialize, Deserialize, BorshSerialize, BorshDeserialize)]
...
impl Header {
    ...
    pub fn direct_parents_slice(&self) -> &[Hash] {
        self.direct_parents()
    }
    ...
    pub fn pre_pow_hash(&self) -> Hash {
        crate::hashing::header::pre_pow_hash(self)
    }
...
impl MemSizeEstimator for Header {
    fn estimate_mem_bytes(&self) -> usize {
        std::mem::size_of::<Self>() + self.parents_by_level.iter().map(|l| l.len()).sum::<usize>() * std::mem::size_of::<Hash>()
    }
}
...
mod tests {
    ...
    #[test]
    fn test_header_direct_parents_and_genesis() {
        let mut header = Header::default();
        assert!(header.direct_parents().is_empty());
        assert!(header.is_genesis());

        let p1 = Hash::from_bytes([1u8; 32]);
        let p2 = Hash::from_bytes([2u8; 32]);
        header.parents_by_level = vec![vec![p1, p2]];

        assert_eq!(header.direct_parents(), &[p1, p2]);
        assert!(!header.is_genesis());
    }
}
