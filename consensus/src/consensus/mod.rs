...
use jio_consensusmanager::{SessionLock, SessionReadGuard};
...
impl ConsensusApi for Consensus {
    ...
    fn get_current_block_color(&self, hash: Hash) -> Option<bool> {
        ...
        while let Some(Reverse(SortableBlock { hash: decedent, .. })) = heap.pop() {
            if self.services.reachability_service.is_chain_ancestor_of(decedent, sink) {
                ...
                // unreachable.
                jio_core::warn!("DAG topology inconsistency: {decedent} is expected to be a merging block of {hash}");
    ...
    fn get_chain_block_samples(&self) -> Vec<DaaScoreTimestamp> {
        ...
        if self.config.net.network_type == NetworkType::Mainnet {
            ...
            // Source: https://github.com/jiogang/jiopad-py-explorer/blob/main/src/tx_timestamp_estimation.ipynb
            // For context see also: https://github.com/jiogang/jiopad-py-explorer/blob/main/src/genesis_proof.ipynb
