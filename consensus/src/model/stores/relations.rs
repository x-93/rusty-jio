...
use jio_consensus_core::{blockhash::BlockHashes, BlockHashMap, BlockHasher, BlockLevel};
use jio_database::prelude::{BatchDbWriter, CachePolicy, DbWriter};
use jio_database::prelude::{CachedDbAccess, DbKey, DirectDbWriter};
use jio_database::prelude::{DirectWriter, MemoryWriter};
use jio_database::prelude::{ReadLock, StoreError};
use jio_database::prelude::{StoreResult, DB};
use jio_database::registry::{DatabaseStorePrefixes, SEPARATOR};
use jio_hashes::Hash;
...
impl RelationsStoreReader for StagingRelationsStore<'_> {
    ...
    fn counts(&self) -> Result<(usize, usize), StoreError> {
        let count = self
            ...
            .iterator()
            .map(|r| r.unwrap().0)
            .map(|k| <[u8; jio_hashes::HASH_SIZE]>::try_from(&k[..]).unwrap())
            .map(Hash::from_bytes)
...
mod tests {
    ...
    use jio_utils::mem_size::MemMode;
    ...
    fn test_db_relations_store() {
        let (lt, db) = create_temp_db!(jio_database::prelude::ConnBuilder::default().with_files_limit(10));
