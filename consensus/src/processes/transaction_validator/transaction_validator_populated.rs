...
use jio_core::warn;
use jio_txscript::{get_sig_op_count, TxScriptEngine};
    ...
    fn check_sequence_lock(tx: &impl VerifiableTransaction, pov_daa_score: u64) -> TxResult<()> {
        ...
        if tx.populated_inputs().filter(|(input, _)| input.sequence & SEQUENCE_LOCK_TIME_DISABLED != SEQUENCE_LOCK_TIME_DISABLED).any(
            |(input, entry)| {
                ...
                //
                // Note: in the jiopad codebase there's a use in i64 in order to use the -1 value
...
mod tests {
    ...
    use jio_consensus_core::tx::{MutableTransaction, PopulatedTransaction, ScriptVec, TransactionId, UtxoEntry};
    use jio_consensus_core::tx::{ScriptPublicKey, Transaction, TransactionInput, TransactionOutpoint, TransactionOutput};
    use jio_txscript_errors::TxScriptError;
