...
use jio_consensus_core::api::stats::BlockCount;
use jio_core::debug;
use jio_notify::subscription::{context::SubscriptionContext, single::UtxosChangedSubscription, Command};
...
pub struct GetBlockTemplateRequest {
    /// Which jio address should the coinbase block reward transaction pay into
...
pub struct GetBlockTemplateResponse {
    ...
    /// Whether jiopad thinks that it's synced.
    /// Callers are discouraged (but not forbidden) from solving blocks when jiopad is not synced.
    /// That is because when jiopad isn't in sync with the rest of the network there's a high
...
//
// This call is only available when this jiopad was started with `--utxoindex`
//
