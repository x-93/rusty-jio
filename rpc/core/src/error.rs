use jio_consensus_core::{subnets::SubnetworkConversionError, tx::TransactionId};
use jio_utils::networking::IpAddress;
...
pub enum RpcError {
    ...
    #[error(transparent)]
    AddressError(#[from] jio_addresses::AddressError),

    #[error(transparent)]
    NetworkTypeError(#[from] jio_consensus_core::network::NetworkTypeError),

    #[error(transparent)]
    NetworkIdError(#[from] jio_consensus_core::network::NetworkIdError),

    #[error(transparent)]
    NotificationError(#[from] jio_notify::error::Error),

    #[error(transparent)]
    MiningManagerError(#[from] jio_mining_errors::manager::MiningManagerError),

    #[error(transparent)]
    ConsensusError(#[from] jio_consensus_core::errors::consensus::ConsensusError),

    #[error(transparent)]
    ScriptClassError(#[from] jio_txscript::script_class::Error),
