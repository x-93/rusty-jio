...
struct RpcTable {
    ...
    jiopad_request_type: Expr,
    jiopad_response_type: Expr,
    payload_ops: Expr,
    handlers: ExprArray,
}
    ...
    fn parse(input: ParseStream) -> Result<Self> {
        ...
        if parsed.len() != 7 {
            return Err(Error::new_spanned(parsed,
                "usage: build_grpc_server_interface!(server_context, ServerContextType, ConnectionType, JiopadRequestType, JiopadResponseType, JiopadPayloadOps, [GetInfo, ..])".to_string()));
        }
        ...
        let jiopad_request_type = iter.next().unwrap().clone();
        let jiopad_response_type = iter.next().unwrap().clone();
...
impl ToTokens for RpcTable {
    fn to_tokens(&self, tokens: &mut TokenStream) {
        ...
        let jiopad_request_type = &self.jiopad_request_type;
        let jiopad_response_type = &self.jiopad_response_type;
        ...
        for handler in self.handlers.elems.iter() {
            ...
            match is_subscription {
                false => {
                    targets.push(quote! {
                        #payload_ops::#handler => {
                            let method: Method<#server_ctx_type, #connection_ctx_type, #jiopad_request_type, #jiopad_response_type> =
                            Method::new(|server_ctx: #server_ctx_type, _: #connection_ctx_type, request: #jiopad_request_type| {
                                Box::pin(async move {
                                    let mut response: #jiopad_response_type = match request.payload {
                ...
                true => {
                    targets.push(quote! {
                        #payload_ops::#handler => {
                            let method: Method<#server_ctx_type, #connection_ctx_type, #jiopad_request_type, #jiopad_response_type> =
                            Method::new(|server_ctx: #server_ctx_type, connection: #connection_ctx_type, request: #jiopad_request_type| {
                                Box::pin(async move {
                                    let mut response: #jiopad_response_type = match request.payload {
                                        Some(Payload::#request_type(ref request)) => {
                                            match jio_rpc_core::#fallback_request_type::try_from(request) {
                                                Ok(request) => {
