...
pub use jio_rpc_macros::build_wrpc_client_interface;
use std::fmt::Debug;
...
impl Inner {
    pub fn new(encoding: Encoding, url: Option<&str>, resolver: Option<Resolver>, network_id: Option<NetworkId>) -> Result<Inner> {
        // log_trace!("Jio wRPC::{encoding} connecting to: {url}");
        ...
        .for_each(|notification_op| {
            ...
            interface.notification(
                notification_op,
                workflow_rpc::client::Notification::new(move |notification: Serializable<jio_rpc_core::Notification>| {
                    ...
                    Box::pin(async move {
                        ...
                        if notification_sender.receiver_count() > 1 {
                            ...
                            log_warn!("WARNING: Jio RPC notification is not consumed by user: {:?}", notification.into_inner());
                        }
                        Ok(())
                    })
                }),
...
/// [`JioRpcClient`] allows connection to the Jio wRPC Server via
...
///
#[derive(Clone)]
pub struct JioRpcClient {
    inner: Arc<Inner>,
}

impl Debug for JioRpcClient {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("JioRpcClient").field("url", &self.url()).field("connected", &self.is_connected()).finish()
    }
}

impl JioRpcClient {
    /// Create a new `JioRpcClient` with the given Encoding, and an optional url or a Resolver.
    /// Please note that if you pass the url to the constructor, it will force the JioRpcClient
    ...
    pub fn new(
        ...
        // FIXME
        // pub fn new(encoding: Encoding, url: &str, ) -> Result<JioRpcClient> {
    ...
    pub fn new_with_args(
        ...
        // let client = JioRpcClient { inner, notifier, notification_mode };
            ...
            loop {
                select! {
                    ...
                    msg = notification_relay_channel.receiver.recv().fuse() => {
                        if let Ok(msg) = msg {
                            // inner.rpc_ctl.notify(msg).await.expect("(JioRpcClient) rpc_ctl.notify() error");
                    ...
                    msg = wrpc_ctl_channel.receiver.recv().fuse() => {
                        if let Ok(msg) = msg {
                            match msg {
                                WrpcCtl::Connect => {
                                    inner.rpc_ctl.signal_open().await.expect("(JioRpcClient) rpc_ctl.signal_open() error");
                                }
                                WrpcCtl::Disconnect => {
                                    inner.rpc_ctl.signal_close().await.expect("(JioRpcClient) rpc_ctl.signal_close() error");
                                }
                            }
                        } else {
