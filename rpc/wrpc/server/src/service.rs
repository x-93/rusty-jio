...
/// ### JioRpcHandler
///
/// [`JioRpcHandler`] is a handler struct that implements the [`RpcHandler`] trait
...
impl JioRpcHandler {
    pub fn new(
        ...
        JioRpcHandler { server: Server::new(tasks, encoding, core_service, options.clone()), options }
    }
}

#[async_trait]
impl RpcHandler for JioRpcHandler {
    ...
    async fn handshake(
        ...
        //     sender,
        //     receiver,
        //     Box::pin(|msg| if msg != "jio" { Err(WebSocketError::NegotiationFailure) } else { Ok(()) }),
        // )
        // .await
...
impl WrpcService {
    ...
    pub fn new(
        ...
        let rpc_handler = Arc::new(JioRpcHandler::new(tasks, *encoding, core_service, options.clone()));
