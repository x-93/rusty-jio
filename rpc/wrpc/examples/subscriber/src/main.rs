// Basic example of a Jio wRPC client that connects to a node
...
// Jio RPC primitives
use jio_wrpc_client::prelude::*;
// reuse wRPC Result type for convenience
use jio_wrpc_client::result::Result;
...
impl Listener {
    pub fn try_new(network_id: NetworkId, url: Option<String>) -> Result<Self> {
        ...
        // Create a basic Jio RPC client instance using Borsh encoding.
        let client = Arc::new(JioRpcClient::new_with_args(WrpcEncoding::Borsh, url.as_deref(), resolver, Some(network_id), None)?);
