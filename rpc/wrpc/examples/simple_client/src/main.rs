// Example of simple client to connect with Jio node using wRPC connection and collect some node and network basic data

use jio_rpc_core::{api::rpc::RpcApi, GetBlockDagInfoResponse, GetServerInfoResponse};
use jio_wrpc_client::{
    client::{ConnectOptions, ConnectStrategy},
    prelude::NetworkId,
    prelude::NetworkType,
    result::Result,
    JioRpcClient, Resolver, WrpcEncoding,
};
...
async fn main() -> ExitCode {
    match check_node_status().await {
        Ok(_) => {
            println!("Well done! You successfully completed your first client connection to Jio node!");
            ExitCode::SUCCESS
        }
        Err(error) => {
...
async fn check_node_status() -> Result<()> {
    ...
    // Verify your Jio node is runnning with --rpclisten-borsh=0.0.0.0:17110 parameter
    ...
    // Define the network your Jio node is connected to
    ...
    let client = JioRpcClient::new(encoding, url, resolver, selected_network, subscription_context)?;
    ...
    // Connect to selected Jio node
    client.connect(Some(options)).await?;

    // Retrieve and show Jio node information
    ...
    // Retrieve and show Jio network information
    ...
    // Disconnect client from Jio node
