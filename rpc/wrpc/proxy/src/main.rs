...
async fn main() -> Result<()> {
    ...
    let jiopad_port = network_type.default_rpc_port();
    ...
    let options = Arc::new(Options {
        ...
        grpc_proxy_address: Some(grpc_proxy_address.unwrap_or_else(|| format!("grpc://127.0.0.1:{jiopad_port}"))),
        verbose,
        // ..Options::default()
    });
    log_info!("");
    ...
    let rpc_handler = Arc::new(JioRpcHandler::new(tasks, encoding, None, options.clone()));
    ...
    log_info!("Jio wRPC server is listening on {}", options.listen_address);
