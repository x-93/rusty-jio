...
use jio_consensus_core::api::counters::ProcessingCounters;
use jio_consensus_core::errors::block::RuleError;
...
use jio_index_core::indexed_utxos::BalanceByScriptPublicKey;
use jio_index_core::{
...
use jio_mining::feerate::FeeEstimateVerbose;
use jio_mining::model::tx_query::TransactionQuery;
use jio_mining::{manager::MiningManagerProxy, mempool::tx::Orphan};
use jio_notify::listener::ListenerLifespan;
use jio_notify::subscription::context::SubscriptionContext;
use jio_notify::subscription::{MutationPolicies, UtxosChangedMutationPolicy};
use jio_notify::{
...
use jio_p2p_flows::flow_context::FlowContext;
use jio_p2p_lib::common::ProtocolError;
use jio_perf_monitor::{counters::CountersSnapshot, Monitor as PerfMonitor};
use jio_rpc_core::{
    api::{
...
use jio_txscript::{extract_script_pub_key_address, pay_to_address_script};
use jio_utils::expiring_cache::ExpiringCache;
use jio_utils::sysinfo::SystemInfo;
use jio_utils::{channel::Channel, triggers::SingleTrigger};
use jio_utils_tower::counters::TowerConnectionCounters;
use jio_utxoindex::api::UtxoIndexProxy;
...
/// A service implementing the Rpc API at jio_rpc_core level.
///
...
pub struct RpcCoreService {
    ...
    fee_estimate_verbose_cache: ExpiringCache<jio_mining::errors::MiningManagerResult<GetFeeEstimateExperimentalResponse>>,
}
...
NOTE: This error usually indicates an RPC conversion error between the node and the miner. If you are on TN11 this is likely to reflect using a NON-SUPPORTED miner.",
    ...
    async fn get_block_template_call(
        ...
        if request.pay_address.prefix != self.config.prefix() {
            return Err(jio_addresses::AddressError::InvalidPrefix(request.pay_address.prefix.to_string()))?;
        }

        // Build block template
        let script_public_key = jio_txscript::pay_to_address_script(&request.pay_address);
    ...
    async fn estimate_network_hashes_per_second_call(
        ...
        const LEGACY_VIRTUAL: jio_hashes::Hash = jio_hashes::Hash::from_bytes([0xff; jio_hashes::HASH_SIZE]);
