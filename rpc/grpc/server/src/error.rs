...
pub enum GrpcServerError {
    ...
    ClosedHandler(JiopadPayloadOps),
...
impl From<GrpcServerError> for jio_rpc_core::error::RpcError {
    fn from(err: GrpcServerError) -> Self {
        match err {
            GrpcServerError::RpcApiError(err) => err,
            GrpcServerError::NotificationError(err) => err.into(),
            _ => jio_rpc_core::error::RpcError::General(err.to_string()),
        }
    }
}

impl From<GrpcServerError> for jio_notify::error::Error {
    fn from(err: GrpcServerError) -> Self {
        match err {
            GrpcServerError::RpcApiError(err) => jio_notify::error::Error::General(err.to_string()),
            GrpcServerError::NotificationError(err) => err,
            _ => jio_notify::error::Error::General(err.to_string()),
        }
    }
}
