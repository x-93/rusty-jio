...
use jio_notify::scope::{NewBlockTemplateScope, Scope};
use jio_rpc_core::api::rpc::RpcApi;
use jio_utils::networking::{ContextualNetAddress, NetAddress};
use std::sync::Arc;

#[tokio::test]
async fn test_client_server_sanity_check() {
    jio_core::log::try_init_logger("info, jio_grpc_core=trace, jio_grpc_server=trace, jio_grpc_client=trace");
