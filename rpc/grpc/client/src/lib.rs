...
use jio_core::{debug, error, trace};
use jio_grpc_core::{
    channel::NotificationChannel,
    ops::JiopadPayloadOps,
    protowire::{jiopad_request, rpc_client::RpcClient, GetInfoRequestMessage, JiopadRequest, JiopadResponse},
...
use jio_utils::{channel::Channel, triggers::DuplexTrigger};
use jio_utils_tower::{
...
impl RpcApi for GrpcClient {
    ...
    //     self.inner.call(JiopadPayloadOps::SubmitBlock, request).await?.as_ref().try_into()
    // }
...
type JiopadRequestSender = async_channel::Sender<JiopadRequest>;
type JiopadRequestReceiver = async_channel::Receiver<JiopadRequest>;
...
/// Data flow:
/// ```
/// //   JiopadRequest -> request_send -> stream -> JiopadResponse
/// ```
///
/// Execution flow:
/// ```
...
struct Inner {
    ...
    // Sending to server
    request_sender: JiopadRequestSender,
    request_receiver: JiopadRequestReceiver,
...
impl Inner {
    ...
    async fn try_connect(
        ...
        request_timeout: u64,
        counters: Arc<TowerConnectionCounters>,
    ) -> Result<(Streaming<JiopadResponse>, ServerFeatures)> {
        // gRPC endpoint
        ...
        // Actual JiopadRequest to JiopadResponse stream
        let mut stream: Streaming<JiopadResponse> = client.message_stream(request_stream).await?.into_inner();
    ...
    async fn call(&self, op: JiopadPayloadOps, request: impl Into<JiopadRequest>) -> Result<JiopadResponse> {
        // Calls are only allowed if the client is connected to the server
        if self.is_connected() {
            let id = u64::from_le_bytes(rand::random::<[u8; 8]>());
            let mut request: JiopadRequest = request.into();
            request.id = id;
    ...
    fn spawn_response_receiver_task(self: Arc<Self>, mut stream: Streaming<JiopadResponse>) {
        // Note: self is a cloned Arc here so that it can be used in the spawned task.
    ...
    async fn start_notify_to_client(&self, scope: Scope) -> RpcResult<()> {
        let request = jiopad_request::Payload::from_notification_type(&scope, Command::Start);
    ...
    async fn stop_notify_to_client(&self, scope: Scope) -> RpcResult<()> {
        if self.handle_stop_notify() {
            let request = jiopad_request::Payload::from_notification_type(&scope, Command::Stop);
            self.call((&request).into(), request).await?;
        }
        Ok(())
    }
