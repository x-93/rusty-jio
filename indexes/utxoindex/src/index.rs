...
use jio_consensus_core::{tx::ScriptPublicKeys, utxo::utxo_diff::UtxoDiff, BlockHashSet};
use jio_consensusmanager::{ConsensusManager, ConsensusResetHandler};
use jio_core::{info, trace};
use jio_database::prelude::{StoreError, StoreResult, DB};
use jio_hashes::Hash;
use jio_index_core::indexed_utxos::BalanceByScriptPublicKey;
use jio_utils::arc::ArcExtensions;
...
const RESYNC_CHUNK_SIZE: usize = 2048; //Increased from 1k (used in go-jiopad), for quicker resets, while still having a low memory footprint.
...
impl UtxoIndexApi for UtxoIndex {
    ...
    fn get_all_outpoints(&self) -> StoreResult<std::collections::HashSet<jio_consensus_core::tx::TransactionOutpoint>> {
