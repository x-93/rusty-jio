...
use jio_txscript::{get_sig_op_count, is_unspendable, script_class::ScriptClass};
...
impl Mempool {
    pub(crate) fn check_transaction_standard_in_isolation(&self, transaction: &MutableTransaction) -> NonStandardResult<()> {
        ...
        // However, in a later version of Jio the consensus-valid transaction version range might diverge from the
...
mod tests {
    ...
    fn test_check_transaction_standard_in_isolation() {
        ...
        let dummy_prev_out = TransactionOutpoint::new(jio_hashes::Hash::from_u64_word(1), 1);
        ...
        let dummy_script_public_key = jio_txscript::pay_to_address_script(&addr);
        let dummy_tx_out = TransactionOutput::new(SOMPI_PER_JIO, dummy_script_public_key);
