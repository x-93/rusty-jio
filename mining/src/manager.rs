...
use jio_consensusmanager::{spawn_blocking, ConsensusProxy};
use jio_core::{debug, error, info, time::Stopwatch, warn};
use jio_mining_errors::{manager::MiningManagerError, mempool::RuleError};
...
impl MiningManager {
                ...
                Err(BuilderError::ConsensusError(BlockRuleError::InvalidTransactionsInNewBlock(invalid_transactions))) => {
                    ...
                    invalid_transactions.iter().for_each(|(x, err)| {
                        ...
                        if let Err(err) = removal_result {
                            ...
                            // https://github.com/jionet/jiopad/issues/1553
    ...
    pub(crate) fn get_realtime_feerate_estimations_verbose(
        ...
        {
            let script_public_key = jio_txscript::pay_to_address_script(&jio_addresses::Address::new(
                prefix,
            ...
            let BlockTemplate { block: jio_consensus_core::block::MutableBlock { transactions, .. }, calculated_fees, .. } =
...
mod tests {
    use super::*;
    use jio_consensus_core::subnets;
    use std::iter::repeat;
