...
use jio_consensus_notify::{root::ConsensusNotificationRoot, service::NotifyService};
use jio_core::{core::Core, debug, info};
use jio_core::{jiopad_env::version, task::tick::TickService};
use jio_database::prelude::CachePolicy;
use jio_grpc_server::service::GrpcService;
use jio_notify::{address::tracker::Tracker, subscription::context::SubscriptionContext};
use jio_rpc_service::service::RpcCoreService;
use jio_txscript::caches::TxScriptCacheCounters;
use jio_utils::git;
use jio_utils::networking::ContextualNetAddress;
use jio_utils::sysinfo::SystemInfo;
use jio_utils_tower::counters::TowerConnectionCounters;

use jio_addressmanager::AddressManager;
use jio_consensus::{consensus::factory::Factory as ConsensusFactory, pipeline::ProcessingCounters};
use jio_consensus::{
...
use jio_p2p_flows::{flow_context::FlowContext, service::P2pService};

use jio_perf_monitor::{builder::Builder as PerfMonitorBuilder, counters::CountersSnapshot};
use jio_utxoindex::{api::UtxoIndexProxy, UtxoIndex};
use jio_wrpc_server::service::{Options as WrpcServerOptions, WebSocketCounters as WrpcServerCounters, WrpcEncoding, WrpcService};

/// Desired soft FD limit that needs to be configured
/// for the jiopad process.
pub const DESIRED_DAEMON_SOFT_FD_LIMIT: u64 = 8 * 1024;
/// Minimum acceptable soft FD limit for the jiopad
/// process. (Rusty Jio will operate with the minimal
...
pub fn get_app_dir() -> PathBuf {
    #[cfg(target_os = "windows")]
    return get_home_dir().join("rusty-jio");
    #[cfg(not(target_os = "windows"))]
    return get_home_dir().join(".rusty-jio");
}
...
/// the application folder that contains jiopad logs and the database.
    ...
    pub fn from_args(args: &Args) -> Self {
        ...
        cfg_if::cfg_if! {
            if #[cfg(feature = "semaphore-trace")] {
                jio_core::log::init_logger(log_dir.as_deref(), &format!("{},{}=debug", args.log_level, jio_utils::sync::semaphore_module_path()));
            } else {
                jio_core::log::init_logger(log_dir.as_deref(), &args.log_level);
            }
        };
...
do you confirm? (answer y/n or pass --yes to the Jiopad command line to confirm all interactive questions)";
    ...
    let mut meta_db = jio_database::prelude::ConnBuilder::default()
    ...
    if !is_db_reset_needed && (args.testnet || args.devnet || args.simnet) {
        ...
        match active_consensus_dir_name {
            Some(dir_name) => {
                let consensus_db = jio_database::prelude::ConnBuilder::default()
    ...
    // Reset Condition: Need to reset if we're upgrading from jiopad DB version
    ...
    {
        let msg =
            "Node database is from a different Jiopad *DB* version and needs to be fully deleted, do you confirm the delete? (y/n)";
        get_user_approval_or_exit(msg, args.yes);

        info!("Deleting databases from previous Jiopad version");
    ...
    if is_db_reset_needed && !args.reset_db {
        ...
        // Reopen the DB
        meta_db = jio_database::prelude::ConnBuilder::default()
    ...
    let perf_monitor = if args.perf_metrics {
        let cb = move |counters: CountersSnapshot| {
            debug!("[{}] {}", jio_perf_monitor::SERVICE_NAME, counters.to_process_metrics_display());
            debug!("[{}] {}", jio_perf_monitor::SERVICE_NAME, counters.to_io_metrics_display());
            #[cfg(feature = "heap")]
            debug!("[{}] heap stats: {:?}", jio_perf_monitor::SERVICE_NAME, dhat::HeapStats::get());
        };
    ...
    let index_service: Option<Arc<IndexService>> = if args.utxoindex {
        ...
        let utxoindex_db = jio_database::prelude::ConnBuilder::default()
