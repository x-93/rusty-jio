...
## WASM32 bindings for Rusty Jio SDK
...
- [**Rust** documentation](https://docs.rs/jio-wasm/latest/jio_wasm/index.html)
- [**TypeScript** documentation](https://jio.aspectron.org/docs/)
...
- **RPC API** — RPC API for the Jio node using WebSockets.
- **Wallet SDK** — Bindings for primitives related to key management and transactions.
- **Wallet API** — API for the Rusty Jio Wallet framework.
...
- `web/jio` - **full** Rusty Jio WASM32 SDK bindings for use in web browsers.
- `web/jio-rpc` - only the RPC bindings for use in web browsers (reduced WASM binary size).
- `nodejs/jio` - **full** Rusty Jio WASM32 SDK bindings for use with NodeJS.
- `docs` - Rusty Jio WASM32 SDK documentation.
...
- `examples/javascript/wallet` - Interfacing with the Rusty Jio Wallet framework.
...
Included documentation in the release can be accessed by loading the `docs/jio/index.html` 
file in a web browser.

## Building from Source

To build the WASM32 SDK from source, you need to have the Rust environment installed. To do that,
follow instructions in the [Rusty Jio README](https://github.com/jionet/rusty-jio).
...
serve them from the root of the SDK folder (`jio-wasm32-sdk` if using a redistributable or
`rusty-jio/wasm` if building from source). This is because examples use relative paths.
...
before loading the WASM32 library. The compatible WebSocket library is [WebSocket](https://www.npmjs.com/package/websocket) and is included in the `jio` NPM package. `jio` package is a wrapper around `jio-wasm` that imports and installs this WebSocket shim in the `globalThis` object and then re-exports `jio-wasm` exports.
...
<html>
    <head>
        <script type="module">
            import * as jio from './jio/jio-wasm.js';
            (async () => {
                await jio.default('./jio/jio-wasm_bg.wasm');
