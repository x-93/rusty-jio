...
(async () => {
    ...
    async function log_transactions(accountId) {
        ...
        transactionsResult.transactions.forEach((tx) => {
            ...
            list.push({
                Id: tx.id,
                Type: tx.data.type,
                Value: sompiToJioString(value(tx) || 0)
            });
    ...
    try {
                ...
                case "balance":
                    ...
                    Object.keys(balance).map(id => {
                        list.push({
                            ...
                            Mature: sompiToJioString(data.balance.mature),
                            Pending: sompiToJioString(data.balance.pending),
                            Outgoing: sompiToJioString(data.balance.outgoing),
        ...
        let sendResult = await wallet.accountsSend({
            walletSecret,
            accountId: firstAccount.accountId,
            priorityFeeSompi: jioToSompi("0.001"),
            destination: [{
                address: firstAccount.changeAddress,
                amount: jioToSompi("1.5")
            }]
        });
        console.log("sendResult", sendResult);

        // Transfer jio between accounts
        let transferResult = await wallet.accountsTransfer({
            ...
            transferAmountSompi: jioToSompi("2.4"),
        });
